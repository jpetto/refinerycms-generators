...
describe ::Refinery::EngineGenerator do
  ...
  let(:engine_generator_root) { Pathname.new(File.expand_path('../../../', __FILE__)) }
  let(:tmp_refinery_app_name) { "tmp_refinery_app" }
  let(:tmp_refinery_app_root) { Pathname.new(File.join(Dir.tmpdir, tmp_refinery_app_name)) }
  let(:app_root) { tmp_refinery_app_root }
